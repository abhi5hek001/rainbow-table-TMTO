...
class RainbowTable:
    ...
    def __init__(self, algorithm, charset, min_length, max_length,
        ...
        self.charset_len = len(self.charset) # Store charset length
        ...
        # --- NEW ---
        # Calculate the total size of the password "keyspace"
        # This is the total number of passwords of length min..max
        self.keyspace_size = 0
        for i in range(self.min_length, self.max_length + 1):
            self.keyspace_size += self.charset_len ** i
    ...
    def reduce_function(self, hashstring, index):
        ...
        given hash string.
        (A robust, textbook implementation to prevent collisions)

        Arguments:
                hashstring {bytes} -- hash to reduce
        ...
        # 1. Convert hash bytes to a large integer
        hash_int = int.from_bytes(hashstring, 'big')

        # 2. "Mix" the index and hash, and map to the password keyspace
        #    This is the core of the reduction.
        #    We add the index to make R_i different from R_j.
        #    We use modulo to map the giant hash_int into our
        #    password space (total number of possible passwords).
        reduced_int = (hash_int + index) % self.keyspace_size
        
        # 3. Determine password length from the reduced integer
        #    We "un-base" the integer to find its length
        pswLength = self.min_length
        temp_size = 0
        for i in range(self.min_length, self.max_length + 1):
            temp_size += self.charset_len ** i
            if reduced_int < temp_size:
                pswLength = i
                break
            
        # 4. Build the new password
        #    This converts the reduced_int into a base-N string,
        #    where N is the length of our charset.
        reduced_value = ""
        temp_int = reduced_int
        
        for _ in range(pswLength):
            char_index = temp_int % self.charset_len
            reduced_value += self.charset[char_index]
            temp_int //= self.charset_len

        # The password chars will be in reverse order, but this
